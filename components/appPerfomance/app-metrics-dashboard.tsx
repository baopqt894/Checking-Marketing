...
export function AppMetricsDashboard({ initialSelectedApp }: AppMetricsDashboardProps) {
  ...
  const handleSelectApp = (appId: string) => {
    // Giữ nguyên selectedPublisherId khi chọn app
    setSelectedApp(appId)
    fetch30DayData(appId)
  }

  const handleBackToOverview = () => {
    // Giữ nguyên selectedPublisherId khi quay lại overview
    setSelectedApp(null)
    setDailyData([])
  }
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        {selectedApp && dailyData.length > 0 ? (
          ...
          dataReady && (
            <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
              <AppOverviewTable apiData={apiData} onSelectApp={handleSelectApp} />
            </div>
          )
        )}
